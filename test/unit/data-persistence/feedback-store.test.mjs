...
describe("feedback-store", () => {
  ...
  describe("writeFeedbackJsonl / readFeedbackJsonl", () => {
    it("writes and reads normalized feedback JSONL", async () => {
    ...
    it("writes and reads comparison feedback JSONL", async () => {
      const dir = await mkdtemp(join(tmpdir(), "ludoforge-"));
      const filePath = join(dir, "feedback.jsonl");
    ...
    it("writes and reads comparison feedback metadata JSONL", async () => {
      const dir = await mkdtemp(join(tmpdir(), "ludoforge-"));
      const filePath = join(dir, "feedback.jsonl");
  ...
  describe("serializeFeedbackRecord", () => {
    it("serialization is deterministic for the same logical feedback record", async () => {
      const recordA = {
        ...ratingFeedback,
        tags: ["balance", "ux"],
        rationale: undefined,
  ...
  describe("validation", () => {
    it("rejects missing required fields and invalid feedback samples", async () => {
      const dir = await mkdtemp(join(tmpdir(), "ludoforge-"));
      const filePath = join(dir, "feedback.jsonl");
