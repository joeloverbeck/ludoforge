...
describe("simulation-run-store", () => {
  ...
  describe("writeSimulationRunJsonl / readSimulationRunJsonl", () => {
    it("writes and reads simulation run JSONL", async () => {
  ...
  describe("writeMetricsJsonl / readMetricsJsonl", () => {
    it("writes and reads metrics JSONL", async () => {
      const dir = await mkdtemp(join(tmpdir(), "ludoforge-"));
  ...
  describe("serializeSimulationRunRecord", () => {
    it("serialization is deterministic for the same logical run record", async () => {
      const recordA = {
        ...runRecord,
        summary: {
          terminationReason: "condition",
