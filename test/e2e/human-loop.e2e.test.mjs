...
test("human loop helper prompts, applies action, and advances turn", async () => {
...
test("human loop re-prompts on invalid input before accepting a valid choice", async () => {
  const definition = await loadFixture("choice-game.json");
  const state = createInitialState(definition);
...
test("mixed participants route to human and AI selectors", async () => {
  const definition = await loadFixture("choice-game.json");
  const state = createInitialState(definition);
