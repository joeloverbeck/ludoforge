...
1. Seed population (generated by default; optional folder/import seeding)
   ...
   - Generated seeds wire all token types and zones to action effects via
     `wireTokenTypesToActions()`, ensuring zero unused elements at birth.
...
8. Evolution operators (optional between generations)
   ...
   - The `unused-element-prune` repair operator strips zones, token types, and
     variables not referenced by any action, trigger, termination, or scoring
     expression (via `collectUsedIds()`). It keeps at least one of each type and
     cascades token-type removal to zones that reference the pruned type.
...
Population size is maintained across generations by four mechanisms:
...
3. **Population cap**: after evolution, the runner truncates to
   `runner.maxPopulationSize` (falls back to `seeding.populationSize`) to
   prevent unbounded growth when `offspringPerParent > 1`.
4. **Population replenishment**: the runner injects random genomes when the
