...
export const subtreeSwapCrossover = {
  ...
  crossover(parentA, parentB, rng) {
    ...
    VERBOSE && process.stderr.write(`[crossover] clone start target=${target.name}\n`);
    const definition = structuredClone(definitionA);
    VERBOSE && process.stderr.write(`[crossover] clone done, applying\n`);
    target.apply(definition, definitionB);
    VERBOSE && process.stderr.write(`[crossover] apply done, validating\n`);

    const validation = validateGenomeDefinition(definition);
    VERBOSE && process.stderr.write(`[crossover] validation done valid=${validation.valid}\n`);
    if (!validation.valid) {
      return null;
    }
...
export function crossoverGenome(parentA, parentB, options = {}) {
  ...
  VERBOSE && process.stderr.write(`[crossover] calling operator=${operator.name} parentA=${parentA?.id ?? "?"} parentB=${parentB?.id ?? "?"}\n`);
  const result = operator.crossover(parentA, parentB, rng);
  VERBOSE && process.stderr.write(`[crossover] operator=${operator.name} returned ${result ? "genome" : "null"}\n`);
  return result;
}
